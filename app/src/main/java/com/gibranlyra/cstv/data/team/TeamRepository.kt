...
@Inject
constructor(private val teamDataSource: TeamDataSource) {
    private val teams = emptyMap<Int, Team>().toMutableMap()

    fun getTeams(team1Id: Int, team2Id: Int): Flow<Result<List<TeamData>>> = flow {
        try {
            teams.getOrElse(team1Id) {
                val teamDataList =
                    teamDataSource.getTeams(team1Id = team1Id, team2Id = team2Id)
                        .map { team ->
                            teams[team.id] = team
                            team.toTeamData()
                        }
